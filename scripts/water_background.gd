...
func _process(delta):
	...
	depths.motion_offset = Vector2(GlobalValues.playerPosition.x / 4 - sin(time/3) * 60, GlobalValues.playerPosition.y / 16 + cos(time/4) * 15)
	abyss.motion_offset = Vector2(GlobalValues.playerPosition.x / 2, GlobalValues.playerPosition.y / 8)
